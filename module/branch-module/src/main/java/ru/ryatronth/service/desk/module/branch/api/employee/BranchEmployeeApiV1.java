...
public interface BranchEmployeeApiV1 {
    ...
    @Operation(summary = "Получить сотрудников филиала",
            description = "Возвращает список сотрудников филиала с пагинацией.")
    @ApiResponse(responseCode = "200",
            description = "Список сотрудников",
            content = @Content(schema = @Schema(implementation = BranchEmployeeDto.class)))
    @GetMapping
    ResponseEntity<Page<BranchEmployeeDto>> getEmployees(
            @Parameter(description = "ID филиала",
                    required = true) @PathVariable UUID branchId,
            @ParameterObject Pageable pageable);

    @Operation(summary = "Назначить сотруднику категорию",
            description = "Привязывает категорию заявок к сотруднику филиала.")
    ...
    ResponseEntity<BranchEmployeeCategoryDto> assignEmployeeCategory(
            ...
            @Parameter(description = "ID сотрудника",
                    required = true) @PathVariable UUID employeeId,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Категория для назначения",
                    required = true,
                    content = @Content(schema = @Schema(
                            implementation = AssignEmployeeCategoryDto.class)))
            @RequestBody AssignEmployeeCategoryDto dto);

    @Operation(summary = "Удалить категорию сотрудника",
            description = "Удаляет категорию заявок, назначенную сотруднику филиала.")
    ...
    ResponseEntity<Void> deleteEmployeeCategory(
            ...
            @Parameter(description = "ID сотрудника",
                    required = true) @PathVariable UUID employeeId,
            @Parameter(description = "ID связи",
                    required = true) @PathVariable UUID categoryId);
